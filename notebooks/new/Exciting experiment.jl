...
begin
	...
	ui_forecast_n = @bind forecast_n Select([2, 10, 20]:30, default=10)
		...
		try
			HTTP.WebSockets.open(BINANCE_API_WS; verbose=false) do ws
				try
					...
					if isa(ex, EOFError)
						@debug "eoferror!" exception=(ex, catch_backtrace())
					else
						rethrow()
					end
				end
...
reset; first_price, set_first_price = @use_state(nothing)
...
variance_estimations = begin
	...
	y_mean, y_ci = pred_y_means[end], pred_y_stds[end] * σ_ci
